...
# NB this code is split because I saved the first three architectures in the old format
all_estimators <- lapply(1:3, function(i) {
  lapply(1:J, function(j) {
    estimator <- architectures[[i]]()
    #TODO might have to update this because of new saving approach... let's see
    loadbestweights(estimator, file.path(savepath, paste0("architecture", i), paste0("estimator", j)))
  })
})
all_estimators <- c(all_estimators, list(lapply(1:J, function(j) {
    i <- 4  
    estimator <- architectures[[i]]()
...
#TODO NeuralEstimators doesn't allow this, because of the way that the arrays are stacked! In the optimised method, I need to check that the arrays are all of the same size. The following code simply overwrites the "optimised" array data method with the general fallback:
juliaEval('
import NeuralEstimators: summarystatistics
